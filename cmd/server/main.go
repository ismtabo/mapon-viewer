...
func main() {
	...
	ssnsSvc := service.NewSessionsService(ssns)
	userRepo := repository.NewMongoUserRepository(collection)
	userSvc := service.NewUserService(userRepo)
	userCtrl := controller.NewUserController(userSvc, ssnsSvc)
	...
	routes := routes.NewRoutes(userCtrl, pagesCtrl, maponCtrl, ssnsSvc, log)
	routes.AddRoutes()
